...
<body class="font-sans antialiased text-gray-900 bg-white overflow-x-hidden">
    ...
    <div class="relative overflow-hidden min-h-screen">
        ...
        <!-- Navigation -->
        <nav class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4 sm:py-6 flex justify-between items-center animate-fade-in-up">
            <div class="flex items-center gap-2 sm:gap-3 shrink-0">
                <img src="{{ asset('images/logo.png') }}" alt="Calorcator Logo" class="h-8 w-8 sm:h-10 sm:w-10 object-contain" onerror="this.src='data:image/svg+xml;base64,PHN2ZyB4bWxucz0iaHR0cDovL3d3dy53My5vcmcvMjAwMC9zdmciIHZpZXdCb3g9IjAgMCAxMDAgMTAwIj48Y2lyY2xlIGN4PSI1MCIgY3k9IjUwIiByPSI0MCIgZmlsbD0iIzAyM2U4YSIvPjwvc3ZnPg=='">
                <span class="text-xl sm:text-2xl font-extrabold text-brandBlue tracking-tight hidden sm:block">Calorcator</span>
            </div>
                ...
                <div class="flex items-center gap-2 sm:gap-4 ml-2">
                    @auth
                        ...
                        <div class="relative" x-data="{ open: false }">
                            <button @click="open = !open" class="flex items-center gap-1 sm:gap-2 text-brandBlue font-bold hover:text-brandOrange transition-colors text-sm sm:text-base">
                    ...
                    @else
                        <a href="{{ route('login') }}" class="text-gray-600 font-medium hover:text-brandBlue transition-colors text-sm sm:text-base whitespace-nowrap">Masuk</a>
                        @if (Route::has('register'))
                            <a href="{{ route('register') }}" class="bg-brandOrange text-white font-semibold px-4 py-2 sm:px-6 sm:py-2.5 rounded-full shadow-lg shadow-brandOrange/30 hover:bg-orange-500 hover:shadow-brandOrange/50 hover:-translate-y-0.5 transition-all duration-300 text-xs sm:text-base whitespace-nowrap">Daftar Gratis</a>
                        @endif
                    @endauth
                </div>
            @endif
